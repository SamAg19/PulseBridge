"""
Cardiology Specialist Fetch.ai Agent

Autonomous agent that provides detailed cardiac analysis using MeTTa reasoning.
...
# Agent configuration
AGENT_NAME = "cardiology-specialist-agent"
AGENT_SEED = os.getenv("CARDIOLOGY_AGENT_SEED", "pulsebridge_cardiology_specialist_seed_phrase")
AGENT_PORT = int(os.getenv("CARDIOLOGY_AGENT_PORT", "8003"))
...
cardiology_knowledge = None

logger.info(f"Cardiology Agent Address: {agent.address}")
logger.info(f"ASI:One Integration: {'ENABLED' if ASIONE_ENABLED else 'DISABLED'}")


# CARDIOLOGY ANALYSIS PROTOCOL

cardiology_protocol = Protocol(name="CardiologyAnalysis", version="1.0.0")


@cardiology_protocol.on_message(model=SpecialistAnalysisRequest, replies={SpecialistAnalysisResponse})
async def handle_analysis_request(ctx: Context, sender: str, msg: SpecialistAnalysisRequest):
    ...
    2. Use MeTTa knowledge base for cardiac analysis
    ...
    try:
        ...
        global cardiology_knowledge
        if cardiology_knowledge is None:
            cardiology_knowledge = get_cardiology_knowledge()
            ctx.logger.info("MeTTa cardiology knowledge loaded (30+ rules)")

        # Use MeTTa reasoning engine
        metta_result = cardiology_knowledge.analyze_symptoms(
        ...
        await ctx.send(
            ...
            SpecialistAnalysisResponse(
                specialty="cardiology",
                diagnosis="Cardiology evaluation needed. Analysis encountered an issue.",
                condition="cardiac_evaluation_needed",
                confidence=0.50,
                urgency_score=0.70,
                risk_level="moderate",
                recommendations=["Seek cardiology consultation", "Monitor symptoms"],
...
async def _generate_patient_friendly_diagnosis(
    ...
    prompt = f"""You are a medical communication assistant. Create a patient-friendly explanation of this cardiac diagnosis.
...
4. Uses proper medical specialty name "cardiologist" (not "heart doctor")
            ...
            response = await client.post(
                ...
                json={
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": "You are a medical communication assistant. Always use proper medical specialty names like 'cardiologist', not informal terms."
                        },
...
# Register protocol
agent.include(cardiology_protocol, publish_manifest=True)
...
async def handle_health_check(ctx: Context, sender: str, msg: AgentHealthCheck):
    ...
    metta_loaded = cardiology_knowledge is not None
...
async def startup(ctx: Context):
    ...
    ctx.logger.info("=" * 60)
    ctx.logger.info("Cardiology Specialist Agent Starting...")
    ctx.logger.info("=" * 60)
    ...
    try:
        cardiology_knowledge = get_cardiology_knowledge()
        ctx.logger.info("MeTTa cardiology knowledge base loaded (30+ rules)")
    except Exception as e:
    ...
    ctx.logger.info("Cardiology Analysis Protocol: ENABLED")
    ctx.logger.info("Ready to analyze cardiac symptoms!")
    ctx.logger.info("=" * 60)
...
async def heartbeat(ctx: Context):
    """Periodic heartbeat"""
    ctx.logger.info("Cardiology Agent: Active and ready")


# RUN AGENT

if __name__ == "__main__":
    logger.info("Starting Cardiology Specialist Fetch.ai Agent...")
    logger.info(f"Agent Address: {agent.address}")
    logger.info("This agent provides cardiac analysis using MeTTa reasoning + ASI:One")
    agent.run()
