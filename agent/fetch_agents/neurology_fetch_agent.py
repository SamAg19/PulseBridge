"""
Neurology Specialist Fetch.ai Agent

Autonomous agent that provides detailed neurological analysis using MeTTa reasoning.
...
from metta.neurology_knowledge import get_neurology_knowledge
import os
...
# Agent configuration
AGENT_NAME = "neurology-specialist-agent"
AGENT_SEED = os.getenv("NEUROLOGY_AGENT_SEED", "pulsebridge_neurology_specialist_seed_phrase")
AGENT_PORT = int(os.getenv("NEUROLOGY_AGENT_PORT", "8004"))
...
neurology_knowledge = None

logger.info(f"Neurology Agent Address: {agent.address}")
logger.info(f"ASI:One Integration: {'ENABLED' if ASIONE_ENABLED else 'DISABLED'}")


# NEUROLOGY ANALYSIS PROTOCOL

neurology_protocol = Protocol(name="NeurologyAnalysis", version="1.0.0")


@neurology_protocol.on_message(model=SpecialistAnalysisRequest, replies={SpecialistAnalysisResponse})
async def handle_analysis_request(ctx: Context, sender: str, msg: SpecialistAnalysisRequest):
    ...
    2. Use MeTTa knowledge base for neurological analysis
    ...
    try:
        ...
        global neurology_knowledge
        if neurology_knowledge is None:
            neurology_knowledge = get_neurology_knowledge()
            ctx.logger.info("MeTTa neurology knowledge loaded (30+ rules)")

        # Use MeTTa reasoning engine
        metta_result = neurology_knowledge.analyze_symptoms(
        ...
        await ctx.send(
            ...
            SpecialistAnalysisResponse(
                specialty="neurology",
                diagnosis="Neurology evaluation needed. Analysis encountered an issue.",
                condition="neurological_evaluation_needed",
                confidence=0.50,
                urgency_score=0.65,
                risk_level="moderate",
                recommendations=["Seek neurologist consultation", "Monitor symptoms", "Document symptom timeline"],
...
async def _generate_patient_friendly_diagnosis(
    ...
    prompt = f"""You are a medical communication assistant. Create a patient-friendly explanation of this neurological diagnosis.
...
4. Uses proper medical specialty name "neurologist" (not "brain doctor" or "nerve doctor")
            ...
            response = await client.post(
                ...
                json={
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": "You are a medical communication assistant. Always use proper medical specialty names like 'neurologist', not informal terms."
                        },
...
# Register protocol
agent.include(neurology_protocol, publish_manifest=True)
...
async def handle_health_check(ctx: Context, sender: str, msg: AgentHealthCheck):
    ...
    metta_loaded = neurology_knowledge is not None
...
async def startup(ctx: Context):
    ...
    ctx.logger.info("=" * 60)
    ctx.logger.info("Neurology Specialist Agent Starting...")
    ctx.logger.info("=" * 60)
    ...
    try:
        neurology_knowledge = get_neurology_knowledge()
        ctx.logger.info("MeTTa neurology knowledge base loaded (30+ rules)")
    except Exception as e:
    ...
    ctx.logger.info("Neurology Analysis Protocol: ENABLED")
    ctx.logger.info("Ready to analyze neurological symptoms!")
    ctx.logger.info("=" * 60)
...
async def heartbeat(ctx: Context):
    """Periodic heartbeat"""
    ctx.logger.info("Neurology Agent: Active and ready")


# RUN AGENT

if __name__ == "__main__":
    logger.info("Starting Neurology Specialist Fetch.ai Agent...")
    logger.info(f"Agent Address: {agent.address}")
    logger.info("This agent provides neurological analysis using MeTTa reasoning + ASI:One")
    agent.run()
