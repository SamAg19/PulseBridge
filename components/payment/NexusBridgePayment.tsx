...
export default function NexusBridgePayment({
  ...
  const tokenAddress = chains[chainId]?.[selectedToken];

  const handleBridgeAndExecute = async () => {
    if (!sdk.isInitialized()) {
      setError('Nexus SDK not initialized. Please connect your wallet first.');
      setStatus('error');
      return;
    }
    ...
    try {
      ...
      const result = await sdk.bridgeAndExecute({
        token: selectedToken as any, // PYUSD, USDC, or USDT
